...
def run_pyscf(
    ...
    if pbc_flag:
        ...
        if linear_dep_thr > 0.0:
            ...
            mf = scf.addons.remove_linear_dep_(mf, threshold=linear_dep_thr)
        # newton solver
        if solver_newton:
        ...
        if len(ghost_atoms_index) != 0:
            logger.info(f"Chosen ghost atom indices are {ghost_atoms_index}.")
            logger.info("The prefix X- is added to the corresponding chemical symbols.")
            for g_ind in ghost_atoms_index:
                chemical_symbols[g_ind] = "X-" + chemical_symbols[g_ind]
        ...
        assert mol.cart is False  # this converter supports only the spherical basis.
...
def cli():
    ...
    parser.add_argument("--basis", help="basis", type=str, default="ccecp-pvtz")
    ...
    parser.add_argument("--dft_xc", help="dft_xc", type=str, default="LDA_X,LDA_C_PZ")
