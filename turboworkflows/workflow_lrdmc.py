...
from pyturbo.utils.utility import get_linenum_fort12
...
class LRDMC_workflow(Workflow):
    ...
    async def async_launch(self):
        ...
        lrdmc_pkl_list=[f"{self.lrdmc_pkl_name}_{i}.pkl" for i in range(self.lrdmc_max_continuation)]

        #####
        # big loop for all the lrdmc continuations
        #####
        if self.lrdmc_rerun or not all([os.path.isfile(os.path.join(self.pkl_dir, lrdmc_pkl)) for lrdmc_pkl in lrdmc_pkl_list]):
            logger.info(f"Start: LRDMC calculation")
            ...
            for icont in range(self.lrdmc_max_continuation):
                ...
                # run part
                ####
                if self.lrdmc_rerun or not os.path.isfile(os.path.join(self.lrdmc_dir, self.lrdmc_pkl)):
                    logger.info(f"{self.lrdmc_pkl} does not exist. or lrdmc_rerun = .true.")

                    if icont==0:
                        self.lrdmc_continuation_flag = False
                        logger.info(f"Run test for estimating steps for achieving the target error bar = {self.lrdmc_target_error_bar}")
                        ## estimated necesary steps per optimization to achieve the target error bar.
                        if self.lrdmc_trial_steps < 40 * self.lrdmc_bin_block + self.lrdmc_bin_block * self.lrdmc_warmupblocks:
                            logger.warning(f"lrdmcsteps = {self.lrdmc_trial_steps} is too small! < 40 * bin_block + bin_block * warmupblocks = {40 * self.lrdmc_bin_block + self.lrdmc_bin_block * self.lrdmc_warmupblocks}")
                            logger.warning(f"lrdmcsteps = {self.lrdmc_trial_steps} is set to 40 * bin_block + bin_block * warmupblocks = {40 * self.lrdmc_bin_block + self.lrdmc_bin_block * self.lrdmc_warmupblocks}")
                            self.lrdmc_trial_steps = 40 * self.lrdmc_bin_block + self.lrdmc_bin_block * self.lrdmc_warmupblocks
                        ...
                        mcmc_steps = get_linenum_fort12(os.path.join(self.lrdmc_dir, "fort.12"))
                        energy, error = lrdmc_genius.energy, lrdmc_genius.energy_error
                        logger.info(f"The error bar of the lrdmc energy {error:.5f} Ha per mcmc step={(mcmc_steps - self.lrdmc_bin_block * self.lrdmc_warmupblocks)}")
                        ...
                        estimated_time_for_1_generation = lrdmc_genius.estimated_time_for_1_generation
                        estimated_time = estimated_time_for_1_generation * lrdmc_steps_estimated_proper
                    ...
                    # manual k points!!
                    #if len(self.lrdmc_kpoints) != 0 and self.lrdmc_twist_average == 2: lrdmc_genius.manual_kpoints=self.lrdmc_kpoints
                    ...
                    while not job_submission_flag:
                        ...
                        await asyncio.sleep(self.sleep_time); os.chdir(self.lrdmc_dir)
                        job_submission_flag, job_number=job.job_submit(submission_script="submit.sh")
                ...
                # Fetch part
                ####
                if self.lrdmc_rerun or not os.path.isfile(os.path.join(self.pkl_dir, self.lrdmc_pkl)):
                    ...
                    with open(os.path.join(self.pkl_dir, self.lrdmc_pkl), "wb") as f:
                        pickle.dump(lrdmc_genius, f)
                    with open(os.path.join(self.pkl_dir, self.lrdmc_latest_pkl), "wb") as f:
                        pickle.dump(lrdmc_genius, f)
            ...
            logger.info(f"Skip: LRDMC calculation")
            self.lrdmc_latest_pkl = f"{self.lrdmc_pkl_name}_latest.pkl"
