...
class VMC_workflow(Workflow):
    ...
    async def async_launch(self):
        ...
        #####
        if self.vmc_rerun or not all(
            [
                os.path.isfile(os.path.join(self.pkl_dir, vmc_pkl))
                for vmc_pkl in vmc_pkl_list
            ]
        ):
            logger.info("Start: VMC calculation")
            ...
            for icont in range(self.vmc_max_continuation):
                ...
                if self.vmc_rerun or not os.path.isfile(
                    ...
                    if icont == 0:
                        ...
                        logger.info(
                            f"The target error bar is {self.vmc_target_error_bar:.5f} Ha"
                        )
                        logger.info(
                    ...
                    vmc_genius.generate_input(
                        ...
                        cont=self.vmc_continuation_flag,
                    )
