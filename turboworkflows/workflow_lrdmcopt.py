...
logger = getLogger('Turbo-Workflows').getChild('lrdmcopt')
...
class LRDMCopt_workflow(Workflow):
    ...
    async def async_launch(self):
        ...
        lrdmcopt_pkl_list=[f"{self.lrdmcopt_pkl_name}_{i}.pkl" for i in range(self.lrdmcopt_max_continuation)]

        #####
        # big loop for all the LRDMCopt continuations
        #####
        if self.lrdmcopt_rerun or not all([os.path.isfile(os.path.join(self.pkl_dir, lrdmcopt_pkl)) for lrdmcopt_pkl in lrdmcopt_pkl_list]):
            logger.info(f"Start: LRDMCopt calculation")
            ...
            for icont in range(self.lrdmcopt_max_continuation):
                ...
                # run part
                ####
                if self.lrdmcopt_rerun or not os.path.isfile(os.path.join(self.lrdmcopt_dir, self.lrdmcopt_pkl)):
                    logger.info(f"{self.lrdmcopt_pkl} does not exist. or lrdmcopt_rerun = .true.")

                    if icont==0:
                        self.lrdmcopt_continuation_flag = False
                        logger.info(f"Run test for estimating steps for achieving the target error bar = {self.lrdmcopt_target_error_bar}")
                        ...
                        with open(os.path.join(self.lrdmcopt_dir, plrdmcopt_pkl), "rb") as f:
                            lrdmcopt_genius = pickle.load(f)
                        logger.info(f"LRDMC optimization, production run step")
                        energy, error = lrdmcopt_genius.energy, lrdmcopt_genius.energy_error
                        logger.info(f"The lrdmc energy at the final step in the test run is {energy[-1]:.5f} Ha")
                        lrdmcopt_pyturbo=LRDMCopt.parse_from_file(file=pinput_file, in_fort10="fort.10", twist_average=self.lrdmcopt_twist_average)
                        ...
                        lrdmcopt_steps_estimated_proper = int((nweight - self.lrdmcopt_warmupblocks * self.lrdmcopt_bin_block) * (error[-1] / self.lrdmcopt_target_error_bar) ** 2)
                        logger.info(f"The target error bar per optstep is {self.lrdmcopt_target_error_bar:.5f} Ha")
                        logger.info(f"The estimated steps to achieve the target error bar is {lrdmcopt_steps_estimated_proper:d} steps")
                        if lrdmcopt_steps_estimated_proper <= self.lrdmcopt_warmupblocks * self.lrdmcopt_bin_block:
                            lrdmcopt_steps_estimated_proper = self.lrdmcopt_warmupblocks * self.lrdmcopt_bin_block + 1
                            logger.warning(f"lrdmcopt_steps_estimated_proper is set to {lrdmcopt_steps_estimated_proper}")
                        estimated_time_for_1_generation = lrdmcopt_genius.estimated_time_for_1_generation
                        estimated_time = estimated_time_for_1_generation * lrdmcopt_steps_estimated_proper * self.lrdmcopt_production_optsteps
                    ...
                    # manual k points!!
                    #if len(self.lrdmcopt_kpoints) != 0 and self.lrdmcopt_twist_average == 2: lrdmcopt_genius.manual_kpoints=self.lrdmcopt_kpoints
                    ...
                    while not job_submission_flag:
                        ...
                        await asyncio.sleep(self.sleep_time); os.chdir(self.lrdmcopt_dir)
                        job_submission_flag, job_number=job.job_submit(submission_script="submit.sh")
                    logger.info("Job submitted.")

                    with open(os.path.join(self.lrdmcopt_dir, self.lrdmcopt_pkl), "wb") as f:
                    ...
                    with open(self.jobpkl, "rb") as f:
                        job=pickle.load(f)
                    with open(os.path.join(self.lrdmcopt_dir, self.lrdmcopt_pkl), "rb") as f:
                        lrdmcopt_genius=pickle.load(f)

                ####
                # Fetch part
                ####
                if self.lrdmcopt_rerun or not os.path.isfile(os.path.join(self.pkl_dir, self.lrdmcopt_pkl)):
                    logger.info(f"{self.lrdmcopt_pkl} does not exist in {self.pkl_dir}.")
                    ...
                    lrdmcopt_genius.plot_energy_and_devmax(output_names=[f"out_fn_opt_{i}" for i in range(icont+1)], interactive=False)
                    if icont>0:
                        with open("forces.dat", 'r') as f:
                            lines = f.readlines()
                        lrdmcopt_done_optsteps = len(lines)
                        optwarmupsteps = int(self.lrdmcopt_optwarmupsteps_ratio * lrdmcopt_done_optsteps)
                        logger.info(f"optwarmupsteps is set to {optwarmupsteps} (the first {self.lrdmcopt_optwarmupsteps_ratio*100:.0f}% steps are disregarded.)")
                        logger.info(f"The final {(1-self.lrdmcopt_optwarmupsteps_ratio)*100:.0f}% steps will be used for averaging parameters.")
                        lrdmcopt_genius.average(optwarmupsteps=optwarmupsteps, input_name=self.input_file, output_names=[f"out_fn_opt_{i}" for i in range(icont+1)], graph_plot=True)
            ...
            logger.info(f"Skip: LRDMCopt calculation")
            self.lrdmcopt_latest_pkl = f"{self.lrdmcopt_pkl_name}_latest.pkl"
